...
class ModelArguments:
    ...
    finetune_terms: str = field(
        default="no",
        metadata={"help": "what terms to train like bitfit (bias). you can write bias;lm_head "},
    )

def main(): 
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"): #sys.argv[0]是文件路径 sys.argv[1] sys.argv[2] 都是输入的参数
